...
export class UserPageComponent implements OnInit {
    ...
    allClasses: Class[] = [// I need to replace this hardcoded data with the real data->getClasses() from the service after api finish
        {
            id: '1', name: 'Class 1', students: [], tutors: [], created: new Date(), updated: new Date(), init: () => {
            }, toJSON: () => ({})
        },
        {
            id: '2', name: 'Class 2', students: [], tutors: [], created: new Date(), updated: new Date(), init: () => {
            }, toJSON: () => ({})
        }
    ...
    onSelectUser(user: User | null, isChecked: boolean): void {
        ...
        if (isChecked) {
            this.selectedUser = {
                ...user,
                init: user?.init || (() => {
                }),  // to avoid the error this is default implementation of init if user is undefined
    ...
    addUser(user: User): void {
        try {
            ...
            this.newUser = {
                ...
                updated: new Date(),
                init: () => {
                }, // I might use it to implement some default actions
                toJSON: () => ({})
            };
